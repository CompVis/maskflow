...
def modulate(x, shift, scale):
    if shift.dim() == 2:
        shift = shift.unsqueeze(1)  # [b*f, 1, D]
...
class TransformerBlock(nn.Module):
    ...
    def _forward(self, x, c):
        ...
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
                self.adaLN_modulation(c).chunk(6, dim=c.dim()-1)
            )
        if c.dim() == 2:
...
class Latte(nn.Module):
    ...
    def forward(
        ...
        :param x: an [N x F x C x H x W] Tensor of inputs.
                N: batch size
                F: number of frames (temporal dimension)
                C: number of channels (e.g., RGB=3)
                H, W: spatial dimensions
        :param t: an [N x F] Tensor of timesteps.
        :param y: an [N] Tensor of labels, if class-conditional.
        :param cond_drop_prob: probability of dropping condition for classifier-free guidance.
        :param use_fp16: whether to use FP16 precision.
        :return: an [N x F x vocab_size x C x H x W] Tensor of outputs.
        """
        N, F, H, W = x.shape
        bs = N
        D = self.hidden_size
        ...
        x = x_embed + self.pos_embed  # [(N*F), num_patches, D]

        if not self.time_cond:
            t = torch.zeros_like(t, dtype=t.dtype, device=x.device)

        t_emb = self.t_embedder(t, use_fp16=use_fp16)  # [N, F, D]
        c_spatial = rearrange(t_emb, "n f d -> (n f) d")  # [(N*F), D]
        c_temp = repeat(t_emb, "n f d -> (n p) f d", p=self.x_embedder.num_patches)  # [(N*num_patches), F, D]

        for i in range(0, len(self.blocks), 2):
            spatial_block, temp_block = self.blocks[i : i + 2]
            x = spatial_block(x, c_spatial)  # [(N*F), num_patches, D]
            x = rearrange(x, "(b f) p d -> (b p) f d", b=bs, f=self.num_frames)  # [(N*num_patches), F, D]

            if i == 0:
                x = x + self.temp_embed

            x = temp_block(x, c_temp)  # [(N*num_patches), F, D]
            x = rearrange(x, "(b p) f d -> (b f) p d", b=bs, f=self.num_frames)

        x = self.final_layer(x, c_spatial)  # [(N*F), num_patches, patch_size**2 * out_channels]
        x = self.unpatchify(x)  # [(N*F), out_channels, H, W]

        x = rearrange(x, "(b f) c h w -> b c f h w", b=bs)  # [N, out_chan
        return x
    ...
    def forward_with_cfg(
        self, x, t, y=None, cfg_scale=7.0, use_fp16=True, text_embedding=None
    ):
        raise NotImplementedError("Not implemented yet")
        """
